  ...
  return {
    ...
    hooks: {
      'astro:config:setup': async ({
        ...
        const buildLogger = logger.fork('trucksync');

        const virtualModuleId = 'trucksync:config';
        ...
        updateConfig({
          ...
          vite: {
            plugins: [
              {
                name: 'vite-plugin-trucksync-config',
                resolveId(id) {
        ...
        if (typeof _themeConfig === 'string') {
          ...
          buildLogger.info(`trucksync \`${_themeConfig}\` has been loaded.`);
        } else {
          buildLogger.info(`trucksync config has been loaded.`);
        }
      },
